...
import utils.textgrid as tgp


def textgrid_2_word_list(textgrid_file, whichTier ='pinyin', utf16 = True):
    """
...
def line_2_word_list(line, entireWordList):
    """
...
def word_lists_parse_by_lines(entireLine, entireWordList):
    """
    ...
    for line in entireLine:
        ...
        if len(asciiLine.replace(" ", "")):                                      # if line is not empty
            numLines += 1
            nestedWordList = []
            wordList = line_2_word_list(line, entireWordList)
...
def syllable_textgrid_extraction(textgrid_path, recording, tier0, tier1):
    """
    ...
    syllableList, _ = textgrid_2_word_list(textgrid_file, whichTier=tier0)
    phonemeList, _ = textgrid_2_word_list(textgrid_file, whichTier=tier1)

    # parse syllables of groundtruth
    nestedPhonemeLists, numSyllables, numPhonemes = word_lists_parse_by_lines(syllableList, phonemeList)
